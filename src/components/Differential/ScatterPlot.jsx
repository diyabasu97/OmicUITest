...
class ScatterPlot extends Component {
  ...
  setupVolcano() {
    ...
    const svg = d3
      ...
      .on('click', () => {
        if (this.props.optionsOpen || this.props.multiFeatureBullpenOpen)
          return;
...
//Adding extra
  removeTooltip() {
    // selectAll, not select: select() removes only the FIRST #tooltip in
    // document order, which lets orphans accumulate (orphans appear when a
    // hovered circle is removed by a re-render before its mouseleave fires).
    d3.selectAll('#tooltip, .volcanoTooltip').remove();
  }
